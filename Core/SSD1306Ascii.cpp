...
void SSD1306Ascii::gotoNexLine() {
    uint8_t row = m_row + m_magFactor;
    setCursor(0, row);
}

void SSD1306Ascii::print(std::string text) {
    writee(text.c_str());
}

void SSD1306Ascii::println(std::string text) {
    print(text);
    gotoNexLine();
}
...
void SSD1306Ascii::clear(uint8_t c0, uint8_t c1, uint8_t r0, uint8_t r1) {
    ...
    for (uint8_t r = r0; r <= r1; r++) {
        setCursor(c0, r);
        writeDisplayBufferedClear();
        for (uint8_t c = c0; c <= c1; c++) {
            ssd1306WriteRamBuf(0);
        }
        writeDisplayBufferedFlush();
    }
    setCursor(c0, r0);
}
//------------------------------------------------------------------------------
...
void SSD1306Ascii::ssd1306WriteRamBuf(uint8_t c) {
    ...
    //writeDisplayBuffered(c, SSD1306_MODE_RAM_BUF);
    m_col++;
}
//------------------------------------------------------------------------------
...
int SSD1306Ascii::writee(uint8_t ch) {
...
int SSD1306Ascii::writee(const char* s) {
    int n = strlen(s);
    for (int i = 0; i < n; i++) {
        writee(s[i]);
    }
